...
*   **An Expert Request (What to Do):**
    > *"I need to add a `status` column to the `orders` table with a default of `'pending'`. **Please follow the `review-migration` skill to do this.**"*
    >
...
*   **For a refactor:**
    > *"This controller is getting too big. I want to extract a service object **using the `refactor-code` skill**."*
...
1. **load-context** — Load the minimum Rails context (schema, routes, nearest pattern, nearest spec). Post the Context Summary before anything else.
2. **plan-tests** — Choose the highest-value first failing spec (request, service, model, job).
3. **write-tests** — Write the failing test and run it.
...
8. **write-yard-docs** — Document new or changed public API.
9. **code-review** — Self-review the full branch diff.
10. **respond-to-review** — When feedback is received: evaluate, push back if wrong, implement one item at a time.
11. **Re-review** — After Critical or significant findings are addressed, re-review before merging.
12. **generate-api-collection** — If the change adds or modifies API endpoints, update the collection.
...
1. **apply-code-conventions** — DRY/YAGNI/PORO/CoC/KISS; project linter as style SoT; structured logging; rules by path (`app/services`, workers, controllers, etc.).
2. **apply-stack-conventions** — Stack-specific defaults (PostgreSQL, Hotwire, Tailwind).

Use **apply-code-conventions** during **code review** and **refactors** as well, not only on greenfield features.
...
1. **code-review**: Systematic review across routing, controllers, models, queries, migrations, security, caching, and testing.
2. **security-check**: Deep dive on auth, params, redirects, output encoding, and secrets.
3. **review-architecture**: Structural review of boundaries, responsibilities, and abstraction quality.
4. **respond-to-review**: When review feedback is received — evaluate, push back if wrong, implement one item at a time.

**Key rules:**

- Use severity levels: Critical / Suggestion / Nice to have
- When receiving feedback: use **respond-to-review** — verify before implementing, no performative agreement
...
- **Bug triage** (`triage-bug`) = diagnosing and reproducing the bug, producing a failing spec
...
1. **triage-bug**: Clarify expected vs actual behavior, narrow the affected layer, identify the highest-value reproduction path.
2. **plan-tests**: Decide the strongest first failing spec for the bug.
...
1. **plan-tests**: Use first when the right starting spec is not obvious. It helps pick the best initial failing spec for request, model, service, job, engine, or bug-fix work.

2. **write-tests**: Covers the full TDD cycle, spec type selection, factory design, and common smells. Includes the Test Feedback and Implementation Proposal checkpoints.

3. **test-service**: Specific patterns for service object tests — instance_double, hash factories, shared_examples.
...
1. **optimize-performance**: Profile first — identify the bottleneck (N+1, slow query, cache miss, allocation). Decide intervention (eager load, index, fragment cache, background move).
2. **apply-code-conventions**: Apply the ActiveRecord performance section (`app/models/**/*.rb`) — eager loading, `exists?`, `pluck`, `find_each`.
3. **write-tests**: Add a regression spec with a query count assertion (`make_database_queries(count: N)`) before optimizing.
...
- Treat GraphQL N+1 as Critical (see **implement-graphql**)
...
1. **implement-authorization**: Decide the right tool, define roles/permissions matrix, plan policy objects per resource.
2. **plan-tests**: Start from the policy spec (allow/deny per role), then controller request specs.
3. **security-check**: IDOR, privilege escalation, and missing controller authorization checks.
...
1. **implement-hotwire**: Pick Frame, Stream, or Stimulus per interaction; define broadcast triggers and target IDs.
2. **apply-stack-conventions**: Stick to Hotwire + Tailwind defaults — no SPA frameworks unless justified.
3. **plan-tests**: System spec for user-visible behavior; request spec for stream broadcast assertions.
...
1. **version-api**: Choose the strategy (URL `:api_version`, `Accept` header, subdomain). Plan the deprecation window and the parallel-run period.
2. **generate-api-collection**: Update or fork the Postman collection per version.
3. **code-review**: Verify back-compat shim, deprecation headers, and changelog entry.
...
1. **seed-database**: Decide seeds vs fixtures vs factories per use case (demo data → seeds; deterministic test rows → fixtures; flexible per-spec data → factories).
2. **write-tests**: Factories must produce minimal valid records; avoid global state.
...
1. **setup-environment**: Walk Docker / docker-compose, env vars, credentials, database create/migrate/seed, test suite, linters, IDE setup.
2. **load-context**: After environment is green, load schema/routes/nearest patterns before the first code change.
...
1. **review-migration**: Plan the strategy — is it reversible? Does it need a phased rollout? Are there zero-downtime constraints?
2. **plan-tests**: Choose a spec for the migration side effects (e.g. column default, data backfill result, index existence).
...
1. **security-check**: Full audit of auth, strong params, IDOR, PII exposure, SQL injection, CSRF, XSS, and secrets handling.
2. Categorize: Critical (fix before merge) vs Suggestion (fix or ticket).
3. **code-review**: Verify fixes are correct and complete.
...
1. **define-domain-language**: Type and field names must match domain language.
2. **implement-graphql**: Schema design — types, mutations, N+1 prevention, authorization, error shape.
3. **plan-tests**: Choose first spec (mutation spec, query spec, or resolver unit).
4. **TDD Feature Loop**: Standard implementation cycle.
5. **security-check**: Introspection disabled, field-level auth, query depth/complexity limits.
...
1. **create-engine**: Choose engine type, set up namespace isolation, define host contract.
2. **test-engine**: Create dummy app, add request/routing/generator specs.
3. **document-engine**: Write README with installation, mounting, configuration, usage (all in English).
4. **create-engine-installer**: Create idempotent install generators.
5. When the engine exposes HTTP endpoints, use **generate-api-collection** to generate or update a Postman Collection (JSON v2.1) for testing.
6. **review-engine**: Review the complete engine for quality.
7. **release-engine**: Prepare versioned release with changelog.
...
1. **write-yard-docs**: Use when writing or reviewing inline docs for Ruby classes and public methods. Apply YARD tags (`@param`, `@option`, `@return`, `@raise`, `@example`) on every public method; keep all text in English. **Required before PR** for new or changed public API.
...
1. **extract-engine**: Identify bounded feature, list host dependencies, create adapters.
2. **refactor-code**: Characterization tests first, then extract in small steps.
3. **create-engine**: Scaffold the engine properly.
4. **test-engine**: Verify behavior is preserved.
...
1. **create-service-object**: Follow `.call` pattern, standardized responses, YARD docs (see **write-yard-docs**), transaction wrapping.
2. **test-service**: Test with subject/let, instance_double, change matchers, error scenarios.

For inline documentation standards, use **write-yard-docs**. For external API integrations, add **integrate-api-client** (Auth/Client/Fetcher/Builder layers).
...
1. **refactor-code**: Define stable behavior, add characterization tests, extract one boundary at a time.
2. **write-tests**: Write the tests that protect the refactoring.
3. **code-review**: Review the refactored code.
