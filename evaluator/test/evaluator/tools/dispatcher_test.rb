  ...
  module Tools
    ...
    class DispatcherTest < Minitest::Test
      def test_execute_read_file
        ...
        result = Dispatcher.call('read_file', '{"path":"test.txt"}', Dir.pwd)
      ...
      def test_execute_write_file
        ...
        result = Dispatcher.call('write_file', '{"path":"new_file.txt", "content":"New text"}', Dir.pwd)

        assert_equal 'success', result
      end

      def test_execute_run_command
        Evaluator::Tools::RunCommand.expects(:call).with('echo test', anything).returns('command output')
        result = Dispatcher.call('run_command', '{"command":"echo test"}', Dir.pwd)
